Mark Morowczynski (Microsoft) and Michael Epping (Microsoft)

© 2021 IDPro, Mark Morowczynski, and Michael Epping
...
Identification is the act of determining which identity is in use or
being interacted with. Before an identity can be authenticated, it must
be determined which identity is being used. A common way this occurs is
through a user providing their username. The username is used to
identify the user, while the password is used to authenticate the user.

Some types of credentials can provide both identification and
authentication simultaneously, such as FIDO credentials or some
biometrics.
...
![A user linked to different authentication factors (password,
biometrics, kerberos ticket) which are in turn linked to cloud service,
local computer, and server.](authn-authz-1.jpg)
...
notification is sent to the device approved by the end-user. Physical
FIDO2 security keys and biometrics like fingerprints and facial
recognition are becoming more common as
well.<a href="#fn2" id="fnref2" class="footnote-ref"><sup>2</sup></a>
...
Authentication is often the first step when an identity wants to access
a resource. We must first determine which identity is trying to access
the resource and determine if it is the legitimate identity or an
imposter. Then we can move on to the next step, determining what access,
if any, should be granted or denied to the identity.
...
decisions a day and not even realize it (unless you get stopped, of
course).
...
of their own
articles.<a href="#fn3" id="fnref3" class="footnote-ref"><sup>3</sup></a>
...
their benefits. They are only authorized to view only their information.
Their manager has a similar view about their own information but
probably also has additional information they can see about their
employees. They can see all the hours worked for their direct employees
but can’t see that about other employees in the organization. Based on
their title, they are only authorized to see that additional information
about their direct reports. Finally, the head of HR might expect to see
a wide range of information about the company. They might expect to see
total hours worked for everyone in the company, total payroll, and
benefits spent. Because they hold the title of Head of HR, they are
authorized to see all this information.
...
backup and restore files on the
system.<a href="#fn4" id="fnref4" class="footnote-ref"><sup>4</sup></a>
...
implementation details of this can be extremely complex. In our above
example of the sales team and engineering team have access to separate
...
minimum permissions necessary to perform their tasks and no more. It is
easy to grant more permissions such that things will work in an effort
to make the authorization process simpler, but we’ll pay the price later
for those decisions, often in catastrophic ways. It’s also often much
more difficult to remove permissions from users and non-human accounts
after they have been implemented. Take the time at the start to ensure
least privilege is being followed for authorization decisions. Your
...
authenticate back to the organization’s sources of
“truth.”<a href="#fn5" id="fnref5" class="footnote-ref"><sup>5</sup></a>
...
assertions to the application/resource provider.
...
included in the assertion. This assertion is signed by the IdP using the
private key of the IdP.

Public key cryptography is used to digitally sign assertions by the IdP
using its private
key.<a href="#fn6" id="fnref6" class="footnote-ref"><sup>6</sup></a> The
application/resource provider uses the public key to validate the
assertion. The application validates that the assertion did come from
the IdP, assuming the private key has not been compromised and the
assertion data has not been modified since it was signed.
...
The setup details vary between federation protocols, but the IdP and the
...
application and IdP
trust.<a href="#fn7" id="fnref7" class="footnote-ref"><sup>7</sup></a>

The application or resource provider is what the user is accessing.
Because the trust and exchange of metadata have happened previously, the
application will trust the signed assertions from the IdP. There are
many thousands of applications and IAM products that support this
federated authentication model.

![An image showing the flow of trust and metadata from the IdP via a
signed claim to the application or resource
provider.](authn-authz-2.jpg)

Figure : The flow of information, trust, and metadata
...
just as crucial; it protects the organization from granting access to
the wrong user or a compromised account, reducing the blast radius of
any nefarious actions as much as possible. Federation via Identity
Providers (IdPs) is a common way to perform this authentication and
authorization today, as applications and services are increasingly found
outside corporate networks. Authentication and authorization techniques
can protect these resources and identities regardless of location.
...
2.  <div id="fn2">
    ...
    <https://fidoalliance.org/fido2/> (accessed 28 September
...
3.  <div id="fn3">
    ...
    [<u>https://bok.idpro.org/article/id/42/</u>](https://bok.idpro.org/article/id/42/),
    and McKee, Mary, “Policy-Based Access Control,” IDPro Body of
    Knowledge, 19 April 2021,
    [<u>https://bok.idpro.org/article/id/61/</u>](https://bok.idpro.org/article/id/61/).<a href="#fnref3" class="footnote-back">↩︎</a>
...
4.  <div id="fn4">
    ...
    <https://bok.idpro.org/article/id/52/>.<a href="#fnref4" class="footnote-back">↩︎</a>
...
5.  <div id="fn5">
    ...
    <https://bok.idpro.org/article/id/45/>.<a href="#fnref5" class="footnote-back">↩︎</a>

    </div>

6.  <div id="fn6">

    For more information on Public-Key Infrastructure, see Sherwood,
    Robert, “Practical Implications of Public Key Infrastructure for
    Identity Professionals, IDPro Body of Knowledge, 30 September 2021,
    <https://bok.idpro.org/article/id/80/>.<a href="#fnref6" class="footnote-back">↩︎</a>
...
7.  <div id="fn7">
    ...
    [<u>https://openid.net/wg/fastfed/</u>](https://openid.net/wg/fastfed/)
    (accessed 31 August
